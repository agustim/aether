                ...
                match msg.text() {
                    ...
                    Some(text) if text.starts_with("/new_intent") => {
                        ...
                        match send_intent_to_server(&http_url, intent).await {
                            Ok(proposal) => {
                                tracing::info!("✅ Proposta #{} generada: {}", proposal.proposal_id, intent);

                                // 2. Aprovar automàticament
                                match auto_approve_proposal(&http_url, proposal.proposal_id).await {
                                    Ok(tasks_added) => {
                                        let response_text = format!(
                                            "✅ **Proposta #{} aprovada**\n\n📋 {} tasques afegides\n\n💬 \"{}\"",
                                            proposal.proposal_id, tasks_added, proposal.original_intent
                                        );
...
mod tests {
    ...
    fn test_integration_response_deserialize() {
        ...
        assert_eq!(response.proposal_id, 1);
        assert_eq!(response.original_intent, "Crear un servidor web");
